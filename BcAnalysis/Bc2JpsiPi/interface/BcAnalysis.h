...
#include "HeavyFlavorAnalysis/BcAnalysis/interface/BcTree.h"



class BcAnalysis : public edm::EDAnalyzer {

   public:
      
      typedef std::vector<std::pair<std::pair<double,double>, std::pair<double,double> > > JTrigDef ;
      typedef std::map<uint, reco::TrackRef  >                                         newTracksDef ;  //aggiunta sara 
 
      explicit	    			BcAnalysis(            const edm::ParameterSet&);
              	   				~BcAnalysis();
