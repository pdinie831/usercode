...
// Class:      AtlasBcStar
// 
/**\class  AtlasBcStar AtlasBcStar.cc HeavyFlavor/AtlasBcStar/src/AtlasBcStar.cc
...
#include "HeavyFlavorAnalysis/AtlasBcStar/interface/AtlasBcStar.h"
...
AtlasBcStar::AtlasBcStar(const edm::ParameterSet& iConfig) :
   ...
   cut_PtPi_          (iConfig.getParameter<double>             ("cut_pt_Pi"        )),
...
//==============================================================================================
AtlasBcStar::~AtlasBcStar()
{
  delete Utils;
}

//=============== method called to for each event ===================================================================
void AtlasBcStar::analyze(const edm::Event& iEvent, const edm::EventSetup& iSetup)
{

  BcStarTree_    ->BcStarTree::BcStarTreeClear();
  runNumber++;
  nEvents_++ ;
  ...
  if(!iEvent.isRealData())  
    BcStarTree_->BcStarTree::SetBcStarTreeGENCand( p_Bc     ,
                                                   p_muP    ,
                                                   p_muM    ,
                                                   p_jpsi   ,
                                                   p_pi     ,
  ...
  for (uint tracksIt =0 ;  tracksIt < tracks->size(); tracksIt++)
  {
     ...
     if (fabs(checkTrk->hitPattern().numberOfValidPixelHits()) < cut_NPixHits_)   continue;                        
     if (fabs(checkTrk->hitPattern().numberOfValidTrackerHits()) < cut_NTrkHits_) continue;                        
     if (checkTrk->normalizedChi2() > cut_TrkChi2_)                               continue;                        
//      if (!iEvent.isRealData() && checkTrk->charge() !=1)            continue;   //per Bc
  ...
  //set BcStarTreeHeader   BcNum=0
  BcStarTree_->BcStarTree::SetBcStarTreeHeader( nEvents_                                ,
...
//---------------------------------------------------------------------------------------------------
  ...
  for(reco::MuonCollection::const_iterator muR1 =recoMuons->begin(); muR1!=recoMuons->end(); ++muR1)
  {
    ...
    if (!recomu1->innerTrack()->hitPattern().trackerLayersWithMeasurement() > 5)                continue;
    if (!recomu1->innerTrack()->hitPattern().pixelLayersWithMeasurement() > 0)                  continue;
    if (!(fabs(recomu1->innerTrack()->dxy(primvtx[0].position())) < 0.3 && 
          fabs(recomu1->innerTrack()->dz(primvtx[0].position())) < 20.))                        continue;
    ...
    for(reco::MuonCollection::const_iterator muR2 =muR1+1; muR2!=recoMuons->end(); ++muR2)
    {
      ...
      if (!recomu2->innerTrack()->hitPattern().trackerLayersWithMeasurement() > 5)              continue;
      if (!recomu2->innerTrack()->hitPattern().pixelLayersWithMeasurement() > 0)                continue;
      if (!(fabs(recomu2->innerTrack()->dxy(primvtx[0].position())) < 0.3 && 
            fabs(recomu2->innerTrack()->dz(primvtx[0].position())) < 20.))                      continue;
...
//         assert(0) ;
      ...
      //set JpsiCand
      BcStarTree_->BcStarTree::AddJpsiCand(  
                                            *jpsiMuP      ,
                                            *jpsiMuM      ,
                                            *jpsiTV       ,
                                            elsigJpsi     ,
...
//----------------------------------------------------------------------------
      ...
      for (newTracksDef::const_iterator it_pi=qualityTracks.begin(); it_pi!=qualityTracks.end(); ++it_pi)
      {
        ...
        RefCountedKinematicParticle theBcCand = BcVertexFitTree->currentParticle();
        ...
        reco::TransientTrack myTrk = theBcCand->refittedTransientTrack();
        ...
        reco::Candidate::LorentzVector Bc_Pi(  theBcCand->currentState().globalMomentum().x(),theBcCand->currentState().globalMomentum().y(), 
                                               theBcCand->currentState().globalMomentum().z(),theBcCand->currentState().kinematicParameters().energy());


        if (Bc_Pi.M() < 5.8  ) continue;
        if (Bc_Pi.M() > 7.   ) continue;
...
//         Matrix33 Covsec  = BcVertex->error().matrix();
        ...
        reco::TrackCollection myPVTracks;
        ...
        for (reco::VertexCollection::const_iterator pvIt = primvtx.begin(); pvIt!=primvtx.end(); pvIt++)        
        {
          ...
          reco::Vertex iiPV = checkPvs;    
          if(!(iiPV.isValid()))                                              continue;

		  reco::Vertex::trackRef_iterator myPVTrkIt;
 
          double idx  = SVPos[0] - iiPV.x();
          double idy  = SVPos[1] - iiPV.y();
          double idz  = SVPos[2] - iiPV.z();
          ...
          if (icos > Cosine)
          {
            ...
            pointPVCl   = iPVCl;
            pointPVtrn  = checkPvs;
			pair<double,double>  pion_IPPairPV = Utils->pionImpactParameter(piTrackCand,iiPV);
			pi_3DIpPointPV               = pion_IPPairPV.first;
			pi_3DIpPointPVSignificance   = pion_IPPairPV.second;
            
//             myPVTracks.clear();
// 			for (myPVTrkIt  = iPV.tracks_begin(); myPVTrkIt != iPV.tracks_end(); ++myPVTrkIt) 
// 			{
// 			  reco::TrackRef trk_now(tracks, (*myPVTrkIt).key());
// 			  myPVTracks.push_back(*trk_now);
// 			}

          }
          
          pair<double,double>  Bc_LongIPPairPV = Utils->LongitudinalIP(myTrk,iiPV);
...
//           pionDz.push_back ( PionTrackCand -> dz( iPV.position()));
        ...
        //cut to reduce computing time!!!!!!!!!
        if (ElsBcLongPV  <= 0    ) continue;    //!!! depend on the chosen PV
        if (Bc_Pi.pt()   <  10   ) continue;
        if (DRPi         >  3.0  ) continue;
        if (longPVCosine <  0.9  ) continue;     //!!! depend on the chosen PV
        ...
        BcStarTree_->BcStarTree::AddBcCand( theNewCand );


//----------------------------------------------------------------------------
        //New loop to find Bc Star candidates
	   if(myPVTracks.size() < 2)                                    continue;
       GlobalPoint BcVertexGP     = BcVertex->position();
	   for (reco::TrackCollection::const_iterator itPVtracks1=myPVTracks.begin(); itPVtracks1!=myPVTracks.end();itPVtracks1++)
	   {
          h1_["LongitPVTrkSize"      ]->Fill(pointPV.tracksSize());
          if (! itPVtracks1->quality(reco::TrackBase::highPurity))                         continue;
// 		  if (fabs(itPVtracks1->hitPattern().numberOfValidPixelHits())   < cut_NPixHits_)  continue;                        
// 		  if (fabs(itPVtracks1->hitPattern().numberOfValidTrackerHits()) < cut_NTrkHits_)  continue;                        
// 		  if (itPVtracks1->normalizedChi2() > cut_TrkChi2_)                                continue;                        
		  bool flagPi1 = false ;                                                         
		  for (reco::MuonCollection::const_iterator mu =recoMuons->begin(); mu!=recoMuons->end(); mu++)                    
		  {                                                                
			if (deltaR(mu->eta(),mu->phi(),itPVtracks1->eta(),itPVtracks1->phi()) < 0.001) { flagPi1=true; break;}                    
		  }                                                                
		  if (flagPi1)                                                                     continue;            
 
		  reco::TransientTrack pi2TrackCand = (*theB).build(*itPVtracks1);
		  if (itPVtracks1->pt() < cut_PtPi_) continue;

		  for (reco::TrackCollection::const_iterator itPVtracks2  = itPVtracks1; itPVtracks2 != myPVTracks.end(); ++itPVtracks2) 
		  {
		    if (itPVtracks2 == itPVtracks1 )                        continue;
			if (itPVtracks1->charge()*itPVtracks2->charge() != -1 ) continue;
			if (itPVtracks2->pt() < cut_PtPi_)                      continue;

            if (! itPVtracks2->quality(reco::TrackBase::highPurity))                         continue;
// 			if (fabs(itPVtracks2->hitPattern().numberOfValidPixelHits())   < cut_NPixHits_)  continue;                        
// 			if (fabs(itPVtracks2->hitPattern().numberOfValidTrackerHits()) < cut_NTrkHits_)  continue;                        
// 			if (itPVtracks2->normalizedChi2() > cut_TrkChi2_)                                continue;                        

			bool flagPi2 = false ;                                                         
			for (reco::MuonCollection::const_iterator mu =recoMuons->begin(); mu!=recoMuons->end(); mu++)                    
			{                                                                
			  if (deltaR(mu->eta(),mu->phi(),itPVtracks2->eta(),itPVtracks2->phi()) < 0.001) { flagPi2=true; break;}                    
			}                                                                
			if (flagPi2)                                                                     continue;            

			const reco::TransientTrack pi3TrackCand = (*theB).build(*itPVtracks2);

//             std::cout << "dr pi1: " << deltaR( itPVtracks1->eta(), itPVtracks1->phi(),Bc_Pi.Eta(), Bc_Pi.Phi())  << std::endl;		
//             std::cout << "dr pi2: " << deltaR( itPVtracks2->eta(), itPVtracks2->phi(),Bc_Pi.Eta(), Bc_Pi.Phi())  << std::endl;		
//             std::cout << "pi1 pt: " << itPVtracks1->pt()  << std::endl;		
//             std::cout << "pi2 pt: " << itPVtracks2->pt()  << std::endl;		

            //Evaluate DCA between pi2 and pi3
//             TrajectoryStateClosestToPoint pi2TS = pi2TrackCand.impactPointTSCP();
//             TrajectoryStateClosestToPoint pi3TS = pi3TrackCand.impactPointTSCP();
//             if (pi2TS.isValid() && pi3TS.isValid()) {
//               ClosestApproachInRPhi cAppPions;
//               cAppPions.calculate(pi2TS.theState(), pi3TS.theState());
//               if (!cAppPions.status()
//                   || cAppPions.distance() > .8) continue;
//             }
             
             
            //Evaluate DCA between pi2 and Bc
            FreeTrajectoryState BcFTState = theBcCand->currentState().freeTrajectoryState(); 
            edm::ESHandle<TransientTrackFromFTSFactory>            theBFromFTS             ;
			const reco::TransientTrack BcTrackCand = (*theBFromFTS).build(BcFTState)       ;
//             TrajectoryStateClosestToPoint BcTS = BcTrackCand.impactPointTSCP()             ;
//             if (pi2TS.isValid() && BcTS.isValid()) {
//               ClosestApproachInRPhi cAppBcPi2;
//               cAppBcPi2.calculate(pi2TS.theState(), BcTS.theState());
//               if (!cAppBcPi2.status()
//                   || cAppBcPi2.distance() > .8) continue;
//             }

            //Evaluate DCA between pi3 and Bc
//             if (pi3TS.isValid() && BcTS.isValid()) {
//               ClosestApproachInRPhi cAppBcPi3;
//               cAppBcPi3.calculate(pi3TS.theState(), BcTS.theState());
//               if (!cAppBcPi3.status()
//                   || cAppBcPi3.distance() > .8) continue;
//             }

			double pipiBcInvMass = Utils->computeInvariantMass(itPVtracks1->px(), itPVtracks1->py(), itPVtracks1->pz(), pion_mass,
			                                                   itPVtracks2->px(), itPVtracks2->py(), itPVtracks2->pz(), pion_mass,
			                                                   Bc_Pi.px(),        Bc_Pi.py(),        Bc_Pi.pz(),        Bc_Pi.M());
			                                                   
            if (pipiBcInvMass > 10) continue;	                                                   			

			std::vector<RefCountedKinematicParticle> XParticles_BcStar;
			XParticles_BcStar.push_back(pFactory.particle(pi2TrackCand, pion_mass,chi,ndf,pion_sigma));
			XParticles_BcStar.push_back(pFactory.particle(pi3TrackCand, pion_mass,chi,ndf,pion_sigma));
			XParticles_BcStar.push_back(pFactory.particle(BcTrackCand,  Bc_Pi.M(),chi,ndf,pion_sigma));
// 			XParticles_BcStar.push_back(theBcCand); 
			
// 			MultiTrackPointingKinematicConstraint *pointingConstr = new MultiTrackPointingKinematicConstraint(BcVertexGP);

            RefCountedKinematicTree bcStarTree = kcvFitterBc.fit(XParticles_BcStar);
//             RefCountedKinematicTree bcStarTree = kcvFitterBc.fit(XParticles_BcStar, pointingConstr);
  
			if (!bcStarTree->isValid()) 
			{
			   h1_["BcStarVtxFitTreeNonValid"      ]->Fill(2);
			   continue;
			} 
            bcStarTree->movePointerToTheTop();
			RefCountedKinematicParticle theBcStarCand = bcStarTree->currentParticle();
			RefCountedKinematicVertex BcStarVertex = bcStarTree->currentDecayVertex();
			if (!BcStarVertex->vertexIsValid())
			{
			   h1_["BcStarVtxNonValid"             ]->Fill(2);
			   continue;
			}

			bcStarTree->movePointerToTheFirstChild();
			RefCountedKinematicParticle Pi2_particle =  bcStarTree->currentParticle();
			bcStarTree->movePointerToTheNextChild();
			RefCountedKinematicParticle Pi3_particle =  bcStarTree->currentParticle();
			bcStarTree->movePointerToTheNextChild();
			RefCountedKinematicParticle BcPostFit_particle =  bcStarTree->currentParticle();

            reco::Candidate::LorentzVector BcStar      ( theBcStarCand->currentState().globalMomentum().x(),  theBcStarCand->currentState().globalMomentum().y(), 
                                                         theBcStarCand->currentState().globalMomentum().z(),  theBcStarCand->currentState().kinematicParameters().energy());
            reco::Candidate::LorentzVector Pi2_LV      ( Pi2_particle->currentState().globalMomentum().x(), Pi2_particle->currentState().globalMomentum().y(), 
                                                         Pi2_particle->currentState().globalMomentum().z(), Pi2_particle->currentState().kinematicParameters().energy());
            reco::Candidate::LorentzVector Pi3_LV      ( Pi3_particle->currentState().globalMomentum().x(), Pi3_particle->currentState().globalMomentum().y(), 
                                                         Pi3_particle->currentState().globalMomentum().z(), Pi3_particle->currentState().kinematicParameters().energy());
            reco::Candidate::LorentzVector BcPostFit_LV( BcPostFit_particle->currentState().globalMomentum().x(),BcPostFit_particle->currentState().globalMomentum().y(), 
                                                         BcPostFit_particle->currentState().globalMomentum().z(),BcPostFit_particle->currentState().kinematicParameters().energy());

            double ClBcStarVtx           = ChiSquaredProbability((double)(BcStarVertex->chiSquared()),(double)(BcStarVertex->degreesOfFreedom()));
            double Bc_BcStarDistance     = vertTool.distance(BcVertex->vertexState(), BcStarVertex->vertexState()).value();
            double Bc_BcStarSignificance = vertTool.distance(BcVertex->vertexState(), BcStarVertex->vertexState()).error();
            double chi2BcVertices        = 0 ;
//             double chi2BcVertices        = vertTool.compatibility(BcVertex->vertexState(), BcStarVertex->vertexState());
            double cosineBcBcstar        = Utils->computeCosine(BcStar.px(), BcStar.py(), BcStar.pz(), Bc_Pi.px(), Bc_Pi.py(), Bc_Pi.pz());

			double BcStarPos[3], BcStarCov[9];
			int nStar = 0;
			BcStarPos[0]    = BcStarVertex->position().x();
			BcStarPos[1]    = BcStarVertex->position().y();
			BcStarPos[2]    = BcStarVertex->position().z(); 
            Matrix33 MyCov ;// = BcStarVertex->error().matrix();
			MyCov(0,0) = BcStarVertex->error().cxx();
			MyCov(1,0) = BcStarVertex->error().cyx();
			MyCov(2,0) = BcStarVertex->error().czx();
			MyCov(0,1) = MyCov(1,0);
			MyCov(1,1) = BcStarVertex->error().cyy();
			MyCov(2,1) = BcStarVertex->error().czy();
			MyCov(0,2) = MyCov(2,0);
			MyCov(1,2) = MyCov(2,1);
			MyCov(2,2) = BcStarVertex->error().czz();

			for (int i=0; i<3; i++)
			{
			  for (int j=0; j<3; j++)
			  { 
				BcStarCov[nStar]    = MyCov(i,j);
				nStar++;
			  }
			}

            TLorentzVector *recoBcStar     = new TLorentzVector(BcStar.px()      , BcStar.py()      , BcStar.pz()      , BcStar.energy()      );
            TLorentzVector *recoBcPostFit  = new TLorentzVector(BcPostFit_LV.px(), BcPostFit_LV.py(), BcPostFit_LV.pz(), BcPostFit_LV.energy());
            TLorentzVector *recoPi2        = new TLorentzVector(Pi2_LV.px()      , Pi2_LV.py()      , Pi2_LV.pz()      , Pi2_LV.energy()      );
            TLorentzVector *recoPi3        = new TLorentzVector(Pi3_LV.px()      , Pi3_LV.py()      , Pi3_LV.pz()      , Pi3_LV.energy()      );

            NumBcStar++;

			//Fill Ntuple - bc star
			BcStarCand theNewStarCand;
			//Bc cand 
			theNewStarCand.SetBcStar               (*recoBcStar                                                                               );
			theNewStarCand.SetBc                   (*recoBcPostFit                                                                            );
			theNewStarCand.SetBcPrefit             (*recoBc                                                                                   );
			theNewStarCand.SetPi1                  (*recoPi2                                                                                  );
			theNewStarCand.SetPi2                  (*recoPi3                                                                                  );

			// Track   
			theNewStarCand.SetPi1Ch                (itPVtracks1->charge()                                                             );
			theNewStarCand.SetPi1PixHits           (itPVtracks1->hitPattern().numberOfValidPixelHits()                                );
			theNewStarCand.SetPi1TrkHits           (itPVtracks1->hitPattern().numberOfValidTrackerHits()                              );
			theNewStarCand.SetPi1NormChi2          (itPVtracks1->normalizedChi2()                                                     );
			theNewStarCand.SetPi1DeltaR            (deltaR(itPVtracks1->eta(),itPVtracks1->phi(),jpsiTV->Eta(),jpsiTV->Phi()) );
			theNewStarCand.SetPi2Ch                (itPVtracks2->charge()                                                              );
			theNewStarCand.SetPi2PixHits           (itPVtracks2->hitPattern().numberOfValidPixelHits()                                 );
			theNewStarCand.SetPi2TrkHits           (itPVtracks2->hitPattern().numberOfValidTrackerHits()                               );
			theNewStarCand.SetPi2NormChi2          (itPVtracks2->normalizedChi2()                                                      );
			theNewStarCand.SetPi2DeltaR            (deltaR(itPVtracks2->eta(),itPVtracks2->phi(),jpsiTV->Eta(),jpsiTV->Phi())   );
			// Bc vertex   
			theNewStarCand.SetCl                   (ClBcStarVtx                                                                               );
			theNewStarCand.SetBcBcStarDistance     (Bc_BcStarDistance                                                                         );
			theNewStarCand.SetBcBcStarSignificance (Bc_BcStarSignificance                                                                     );
			theNewStarCand.SetChi2BcVertices       (chi2BcVertices                                                                            );
			theNewStarCand.SetCosine               (cosineBcBcstar                                                                            );
   
			//Vertices
			for (int i=0; i < 3; i++){
			  theNewStarCand.SetBcVtxPosition     (i, BcStarPos[i]                                                                            );
			}
			for (int i=0; i < 9; i++){
			  theNewStarCand.SetBcVtxCovariance   (i, BcStarCov[i]                                                                            );
			}

            BcStarTree_->BcStarTree::AddBcStarCand( theNewStarCand );
          }
        } // end loop on pions for bc star
...
void AtlasBcStar::beginJob()
{
    ...
    TDirectory * BcStarTreeDir  = file      ->mkdir("BcStarTreeDir" ) ;
    ...
    file->cd() ;
    BcStarTreeDir->cd() ;
    t1_["BcStarTree"] = new TTree("BcStarTree","BcStarTree");
    BcStarTree_       = new BcStarTree();
    t1_["BcStarTree"] -> Branch("BcBr","BcStarTree",&BcStarTree_,64000,2);
    t1_["BcStarTree"] -> SetAutoSave(50000000);
...
//------Histograms for MC--------------------------------------------  
    ...
    h1_["Pion2_IPsignificanceBc"            ] = new TH1F("Pion2_IPsignificanceBc"           ,"Pion2_IPsignificanceBc"                ,    1000,   0, 100   );
    h1_["Pion3_IPsignificanceBc"            ] = new TH1F("Pion3_IPsignificanceBc"           ,"Pion3_IPsignificanceBc"                ,    1000,   0, 100   );



    h1_["BcStarVtxFitTreeNonValid"          ] = new TH1F("BcStarVtxFitTreeNonValid"         ,"BcStarVtxFitTreeNonValid"              ,       4,   0,   4   );
    h1_["BcStarVtxNonValid"                 ] = new TH1F("BcStarVtxNonValid"                ,"BcStarVtxNonValid"                     ,       4,   0,   4   );
    h1_["LongitPVTrkSize"                   ] = new TH1F("LongitPVTrkSize"                  ,"LongitPVTrkSize"                       ,     100,   0, 100   );
...
//===============================================================================================
void AtlasBcStar::MonteCarloStudies(const edm::Event& iEvent)
{
...
//============================================================================================== 
void AtlasBcStar::checkHLT(const edm::Event& iEvent)
{
